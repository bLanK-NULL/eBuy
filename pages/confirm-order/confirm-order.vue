	...
	export default {
		...
		computed:{
			...mapState({
				...
				orderNum:state=>state.orderManager.orderNum,
		...
		onLoad(e) {
			...
			//初始化地址
			this.$store.dispatch('initPathFn')
			// console.log("this.defaultPath",this.defaultPath);
			//如果有默认地址的一个赋值
			if(this.defaultPath!= undefined){
		...
		methods: {
			...
			//确认支付
			goPayment(){
				// console.log("this.$store.state.orderManager.orderNum",this.$store.state.orderManager.orderNum);
				$http.request({
					url: '/submitOrder',
					method: "POST",
					header: {
					...
					data: {
						...
						selectListId: this.selectedList
					}
				}).then(res=>{
					if(res.success)
						uni.navigateTo({
